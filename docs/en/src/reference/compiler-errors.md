...
| Code | Message patterns |
|------|------------------|
| TYP-001 | `Type mismatch: Function expects pointer type (e.g. str) but got {type} from expression {expr}`; `Type mismatch: dereference expects Ptr(T), got {actual}` |
...
| TYP-006 | `Type mismatch: cannot assign {rhs} to fixed binding `{name}` of type {ty}`; `Type mismatch: list element has {actual}, expected {elem}`; `Type mismatch: cannot assign {actual} to dereference of type {expected}` |
