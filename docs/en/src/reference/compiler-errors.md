...
| Code | Message patterns |
|------|------------------|
| TYP-001 | `Type mismatch: Function expects pointer type (e.g. str) but got {type} from expression {expr}`; `Type mismatch: dereference expects Ptr(T), got {actual}`; `Type mismatch: pointer offset expects usize or a non-negative integer literal, got {actual}` |
