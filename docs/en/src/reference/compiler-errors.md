...
| SEM-011 | `Undefined type: {name}`; `` `Self` is only valid in struct field type annotations ``; `unknown type `{legacy}`; use {replacement}` (`int`→`i64`, `list`→`List(T)`, `err`→`Label(:error, Any)`, `atom`/`label`→`Label`, …); `List requires exactly one type argument`; `Process requires exactly one type argument`; `Label application must be Label or Label(:name, T)`; `unknown type constructor `{name}`; builtin constructors are List(T), Process(T), Label(:name, T)`; `{Name} does not take type arguments` |
