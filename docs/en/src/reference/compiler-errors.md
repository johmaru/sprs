...
| Code | Message patterns |
|------|------------------|
| TYP-001 | `Type mismatch: Function expects pointer type (e.g. str) but got {type} from expression {expr}`; `Type mismatch: dereference expects Ptr(T), got {actual}`; `Type mismatch: ordinary read through Ptr(MaybeUninit(T)) is not allowed; use @ref or @take`; `Type mismatch: assignment through Ptr(MaybeUninit(T)) is not allowed; use @init`; `Type mismatch: @{init,ref,take} expects Ptr(MaybeUninit(T)), got {actual}`; `Type mismatch: pointer offset expects usize or a non-negative integer literal, got {actual}` |
...
| TYP-006 | `Type mismatch: cannot assign {rhs} to fixed binding `{name}` of type {ty}`; `Type mismatch: list element has {actual}, expected {elem}`; `Type mismatch: cannot assign {actual} to dereference of type {expected}`; `Type mismatch: cannot initialize dereference of type {expected} with {actual}` |
| TYP-007 | `Type mismatch: argument {n} of `{fn}` expects {ty}, found {actual}`; `Type mismatch in call to `{fn}`: type parameter `{T}` was already resolved to `{ty}`, but the argument has type `{actual}`; `type parameter `{T}` was already resolved to `{ty}`, but the argument has type `{actual}`; `cannot infer generic type `{T}` in call to `{fn}`; `method call requires a struct receiver, found {ty}`; `Type mismatch in call to `{fn}`: multiple `when` rules matched`; unresolved type parameter |
...
| SEM-005 | (removed; old struct `@init` is gone. Pointer `@init(*p, value)` is SEM-013) |
...
| SEM-011 | `Undefined type: {name}`; `` `Self` is only valid in struct field type annotations ``; `unknown type `{legacy}`; use {replacement}` (`int`→`i64`, `list`→`List(T)`, `err`→`Label(:error, Any)`, `atom`/`label`→`Label`, …); `List requires exactly one type argument`; `Ptr requires exactly one type argument`; `MaybeUninit requires exactly one type argument`; `Process requires exactly one type argument`; `Label application must be Label or Label(:name, T)`; `{Name} does not take type arguments`; ``generic struct `{Pair}` expects {n} type argument(s), found {m}``; ``duplicate type parameter `{T}` in struct `{Pair}` ``; ``builtin type name `{name}` cannot be used as a type parameter``; ``unresolved type parameter `{T}` while specializing `{Pair(T)}` ``; ``cannot specialize `{Pair(i64)}`: {reason}``; ``generic specialization expands recursively: Grow(i64) -> Grow(List(i64))``; `recursive struct has infinite storage size`; `generic struct field defaults are not supported in Phase 1; initialize the field explicitly`; ``generic function `{foo}` expects {n} type argument(s), found {m}``; ``duplicate type parameter `{T}` in function `{foo}` `` |
| SEM-013 | Macro arity (`list_push expects 2 arguments`, `buf_len expects 1 argument`, `buf_get expects 2 arguments`, `buf_set expects 3 arguments`, `@clone expects 1 argument`, `@move expects 1 argument`, `@move expects a variable argument`, `@move does not accept a dereference place; use @take for raw storage`, `@init expects 2 arguments`, `@init first argument must be a dereference place`, `@ref expects 1 argument`, `@ref first argument must be a dereference place`, `@take expects 1 argument`, `@take first argument must be a dereference place`, `@cast expects 2 arguments`, `@fcast expects exactly 1 argument`, `@lshift expects 2 arguments (value, shift_amount)`, `@rshift expects 2 arguments (value, shift_amount)`, `@not expects 1 argument`); `@raw` / `@free` require an unsafe block; `Undefined variable: {name}`; `Module '{name}' not found`; `Function '{fn}' not found in module '{module}'`; `Undefined struct : {name}`; `Field '{field}' not found in struct '{name}'`; `Field definition for '{field}' not found in struct '{name}'`; `unknown field \`{field}\` in init {Type}`; `duplicate field \`{field}\` in init {Type}`; `missing required field \`{field}\` in init {Type}` |
| SEM-015 | `Undefined function: {name}` (including a private method called from an importer); `` `@raw` requires an unsafe block ``; `` `@free` requires an unsafe block `` |
