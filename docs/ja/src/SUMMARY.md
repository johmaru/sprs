# 目次

[はじめに](README.md)

- [入門](getting-started.md)

# 言語

- [型と束縛](language/types-and-bindings.md)
- [関数と制御フロー](language/functions-and-control-flow.md)
- [列挙型と構造体](language/enums-and-structs.md)
- [ラベルと match](language/labels-and-match.md)
- [エラー](language/errors.md)
- [Buffer と unsafe](language/buffers-and-unsafe.md)
- [演算子](language/operators.md)

# リファレンス

- [プロジェクト設定](reference/project-config.md)
- [組み込みマクロ](reference/built-in-macros.md)
- [モジュール](reference/modules.md)
- [メモリ管理](reference/memory-management.md)
- [コンパイルエラー](reference/compiler-errors.md)


# 内部実装

- [ランタイム関数](internals/runtime-functions.md)

---

[貢献](contributing.md)
