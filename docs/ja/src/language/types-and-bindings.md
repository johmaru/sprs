...
型の適用はコンパイル時だけです: `List(i64)`、`Process(str)`、`Label(:ok, i64)`、`Label(:error, Any)`、および見えるジェネリック構造体（`Pair(i64)`）。組込みコンストラクタの引数個数規則は従来どおりです（`List(i64, str)`、`Process()`、`Range(i64)` は `SPRS-SEM-011`）。見えるジェネリック構造体の引数個数誤りも `SPRS-SEM-011` です。未知のコンストラクタ名は未定義型です。`Type::Param` はコンパイル時の置換だけに現れ、実行時タグではありません。
