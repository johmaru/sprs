...
- ポインタ加算: `Ptr(T) + offset` は要素単位。stride は LLVM target ABI の `size_of(StorageRep(T))`（struct の padding を含む）。`Ptr(MaybeUninit(T))` の stride も `T` と同じ。`offset` は `usize` または非負の整数リテラル。オーバーフローは `Pointer arithmetic overflow` で panic する。`integer + Ptr(T)`、負リテラル、`Ptr(T)` の `-` `*` `/` `%` は `SPRS-TYP-001`。
