...
| SEM-005 | （削除。旧構造体 `@init` は廃止。ポインタ `@init(*p, value)` は SEM-013） |
...
| SEM-015 | `Undefined function: {name}`（import 先から呼んだ private method を含む）；`` `@raw` requires an unsafe block ``；`` `@free` requires an unsafe block `` |
